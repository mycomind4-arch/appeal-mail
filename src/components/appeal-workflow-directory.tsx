...
import { APPEAL_CATALOG, type AppealWorkflowEntry } from "@/domain/appeal-catalog";
...
const canonicalEntryByWorkflowId = new Map<string, AppealWorkflowEntry>();
for (const entry of APPEAL_CATALOG) {
  const workflowId = entry.workflowRoute.replace(/^\/workflows\//, "");
  if (!canonicalEntryByWorkflowId.has(workflowId)) canonicalEntryByWorkflowId.set(workflowId, entry);
}
  ...
  return <div>
    ...
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16">
      <div className="space-y-12">{categories.filter((c) => category === "ALL" || c === category).map((cat) => { const items = filtered.filter((w) => categoryFor(w.id) === cat); if (!items.length) return null; return <section key={cat}><div className="mb-5 flex items-center gap-3"><h3 className="font-mono text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">{cat}</h3><span className="h-px flex-1 bg-rule/60" /><span className="font-mono text-xs text-muted-foreground">{items.length}</span></div><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{items.map((workflow) => {
        ...
        const catalogEntry = canonicalEntryByWorkflowId.get(workflow.id);
        return catalogEntry ? (
          <Link key={workflow.id} to="/appeal/$slug" params={{ slug: catalogEntry.slug }} className="group flex h-full flex-col overflow-hidden rounded-2xl border border-rule/80 bg-card shadow-card transition-all duration-200 hover:-translate-y-1 hover:border-ink/20 hover:shadow-premium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stamp/50">
            <WorkflowCardContent workflow={workflow} img={img} cat={cat} />
          </Link>
        ) : (
          <Link key={workflow.id} to="/workflows/$workflowId" params={{ workflowId: workflow.id }} className="group flex h-full flex-col overflow-hidden rounded-2xl border border-rule/80 bg-card shadow-card transition-all duration-200 hover:-translate-y-1 hover:border-ink/20 hover:shadow-premium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stamp/50">
            <WorkflowCardContent workflow={workflow} img={img} cat={cat} />
          </Link>
        );
      })}</div></section>; })}</div>
...
function WorkflowCardContent({ workflow, img, cat }: { workflow: (typeof workflows)[string]; img: string; cat: string }) {
  return <>
