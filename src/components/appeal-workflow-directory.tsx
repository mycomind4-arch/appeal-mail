...
export function AppealWorkflowDirectory() {
  ...
  return <div>
    ...
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16">
      <div className="space-y-12">{categories.filter((c) => category === "ALL" || c === category).map((cat) => { const items = filtered.filter((w) => categoryFor(w.id) === cat); if (!items.length) return null; return <section key={cat}><div className="mb-5 flex items-center gap-3"><h3 className="font-mono text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">{cat}</h3><span className="h-px flex-1 bg-rule/60" /><span className="font-mono text-xs text-muted-foreground">{items.length}</span></div><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{items.map((workflow) => <Link key={workflow.id} to={`/workflows/${workflow.id}`} className="group flex h-full flex-col rounded-xl border border-rule bg-card p-5 transition-all hover:-translate-y-0.5 hover:border-ink/30 hover:shadow-md"><div className="flex items-start justify-between gap-4"><span className="rounded-full border border-rule bg-paper px-2.5 py-1 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{cat}</span><span className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 font-mono text-[10px] uppercase tracking-widest text-stamp"><CheckCircle2 size={10} /> Available</span></div><h3 className="mt-4 font-serif text-2xl leading-tight">{workflow.title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{workflow.description}</p><div className="mt-4 border-t border-rule/60 pt-4"><div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Primary search intent</div><div className="mt-1 text-sm font-medium">{workflow.primaryKeyword || "Specialized appeal response"}</div></div><div className="mt-4 flex items-center justify-between"><span className="text-sm font-medium text-stamp">Start workflow</span><ArrowRight size={16} className="text-muted-foreground transition-transform group-hover:translate-x-1" /></div></Link>)}</div></section>; })}</div>
    </div>
  </div>;
}
