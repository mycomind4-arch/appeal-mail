...
export function AppealWorkflowDirectory() {
  ...
  const categories = Array.from(new Set(entries.map((w) => w.category))).sort();
  const filtered = useMemo(() => entries.filter((w) => {
    ...
    return (!query || haystack.includes(query.toLowerCase())) && (category === "ALL" || w.category === category);
      ...
      <div className="space-y-12">{categories.filter((c) => category === "ALL" || c === category).map((cat) => {
        ...
        return <section key={cat}>
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {items.map((w) => {
              const img = getWorkflowHeroImage(w.id) || PLACEHOLDER_IMAGE;
              return <Link key={w.id} to="/workflows/$workflowId" params={{ workflowId: w.id }} className="group flex h-full flex-col overflow-hidden rounded-2xl border border-rule/80 bg-card shadow-card transition-all duration-200 hover:-translate-y-1 hover:border-ink/20 hover:shadow-premium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stamp/50">
                <div className="relative aspect-[16/9] overflow-hidden border-b border-rule/60 bg-paper-deep"><img src={img} alt="" aria-hidden="true" loading="lazy" className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-[1.03]" /><div className="absolute inset-0 bg-gradient-to-t from-ink/60 via-ink/10 to-transparent" /><span className="absolute bottom-3 left-4 rounded-full border border-paper/30 bg-ink/65 px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.16em] text-paper backdrop-blur-sm">{w.category}</span><span className="absolute bottom-3 right-4 flex h-8 w-8 items-center justify-center rounded-full border border-paper/25 bg-ink/60 text-paper backdrop-blur-sm transition-transform group-hover:translate-x-0.5"><ArrowRight size={14} /></span></div>
                <div className="flex flex-1 flex-col p-5 sm:p-6">
                  ...
                  <h3 className="font-serif text-[1.55rem] leading-[1.12] tracking-[-0.01em] text-foreground">{w.title}</h3>
                  <p className="mt-3 flex-1 text-sm leading-6 text-muted-foreground">{w.description}</p>
                  <div className="mt-5 border-t border-rule/60 pt-4"><div className="font-mono text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Search intent</div><div className="mt-1.5 text-xs leading-5 text-ink-soft">{w.primaryKeyword || w.title}</div></div>
                  <div className="mt-5 flex items-center justify-between text-sm font-medium"><span>Open workflow</span><span className="text-stamp transition-transform group-hover:translate-x-1">→</span></div>
                </div>
              </Link>;
            })}
          </div>
