...
async function addTextPart(target: PDFDocument, text: string, title: string): Promise<number> {
  ...
  for (const line of wrapText(text, maxWidth, font, bodySize)) {
    if (y < margin + lineHeight) { page = target.addPage([width, height]); pageCount += 1; y = height - margin; }
...
async function appendPdf(target: PDFDocument, bytes: Uint8Array, ops: PacketPageOp[], partId: string): Promise<number> {
  ...
  const selected = ops.length ? ops.filter((op) => !op.removed).filter((op) => op.partId === partId) : source.getPageIndices().map((i) => ({ partId, sourcePageIndex: i, rotation: 0, removed: false }));
  if (!selected.length) throw new Error(`Packet part ${partId} has no pages selected.`);
  let count = 0;
  for (const op of selected) {
    if (!Number.isInteger(op.sourcePageIndex) || op.sourcePageIndex < 0 || op.sourcePageIndex >= sourceCount) throw new Error(`Invalid page ${op.sourcePageIndex + 1} for packet part ${partId}.`);
    const [page] = await target.copyPages(source, [op.sourcePageIndex]);
    if ((op.rotation || 0) % 360 !== 0) page.setRotation(degrees((op.rotation || 0) % 360));
    target.addPage(page); count += 1;
  }
  return count;
}
async function appendImage(target: PDFDocument, bytes: Uint8Array, mimeType: string): Promise<number> {
  const page = target.addPage([612, 792]); const margin = 36; const maxW = 612 - margin * 2; const maxH = 792 - margin * 2;
  const image = mimeType === "image/png" ? await target.embedPng(bytes) : await target.embedJpg(bytes);
  const scale = Math.min(maxW / image.width, maxH / image.height, 1); const width = image.width * scale; const height = image.height * scale;
  page.drawImage(image, { x: (612 - width) / 2, y: (792 - height) / 2, width, height }); return 1;
}
...
export async function buildPacket(parts: PacketPart[], pageOps: PacketPageOp[] = []): Promise<BuiltPacket> {
  ...
  for (const part of parts) {
    if (part.type === "ai_response") {
      ...
      finalDraftHash = await hashText(part.text); const pages = await addTextPart(target, part.text, part.title || "Appeal Response");
      const bytes = new TextEncoder().encode(part.text); partHashes.push({ id: part.id, type: part.type, sha256: await sha256(bytes), pageCount: pages }); pageCount += pages; continue;
    }
    if (part.type === "uploaded_document") {
      const safeName = sanitizeFilename(part.filename); const validation = validateFile({ filename: safeName, mimeType: part.mimeType, size: part.bytes.byteLength });
      if (!validation.ok) throw new Error(validation.error.message);
      if (part.mimeType === "application/pdf") {
        const dangerousTokens = scanPdfForDangerousTokens(part.bytes); if (dangerousTokens.length) throw new Error(`Uploaded PDF contains blocked active-content tokens: ${dangerousTokens.join(", ")}`);
        const pages = await appendPdf(target, part.bytes, pageOps, part.id); partHashes.push({ id: part.id, type: part.type, sha256: await sha256(part.bytes), pageCount: pages, filename: safeName }); pageCount += pages;
      } else if (part.mimeType === "image/png" || part.mimeType === "image/jpeg") {
        const pages = pageOps.length && pageOps.every((op) => op.partId !== part.id || op.removed) ? 0 : await appendImage(target, part.bytes, part.mimeType); if (!pages) throw new Error(`Packet part ${part.id} has no active page.`);
        partHashes.push({ id: part.id, type: part.type, sha256: await sha256(part.bytes), pageCount: pages, filename: safeName }); pageCount += pages;
      } else throw new Error(`Unsupported packet attachment type: ${part.mimeType}`);
      continue;
    }
    const dangerousTokens = scanPdfForDangerousTokens(part.bytes); if (dangerousTokens.length) throw new Error(`Generated PDF contains blocked active-content tokens: ${dangerousTokens.join(", ")}`);
    const pages = await appendPdf(target, part.bytes, pageOps, part.id); partHashes.push({ id: part.id, type: part.type, sha256: await sha256(part.bytes), pageCount: pages, filename: sanitizeFilename(part.filename) }); pageCount += pages;
  }
