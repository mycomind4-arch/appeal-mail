...
function wrapLine(line: string, maxChars = 88): string[] {
  ...
  for (const word of words) { if (!current) { current = word; continue; } if (current.length + 1 + word.length <= maxChars) current += ` ${word}`; else { lines.push(current); current = word; } }
  if (current) lines.push(current); return lines;
}
export function textToPdf(text: string, title = "MailMyPDF Document"): Buffer {
  const rawLines = [title, "", ...text.split(/\r?\n/).flatMap((line) => wrapLine(line))];
  const pages: string[][] = []; const pageLines = 52;
  for (let i = 0; i < rawLines.length; i += pageLines) pages.push(rawLines.slice(i, i + pageLines));
  if (!pages.length) pages.push([title]);
  const objects: string[] = []; const pageObjectIds: number[] = []; const fontObjectId = 3;
  objects[1] = "<< /Type /Catalog /Pages 2 0 R >>"; objects[2] = ""; objects[fontObjectId] = "<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>";
  let nextId = 4;
  for (const page of pages) {
    const pageId = nextId++; const contentId = nextId++; pageObjectIds.push(pageId);
    const commands = ["BT", "/F1 10 Tf", "50 742 Td", "14 TL"];
    page.forEach((line, index) => { if (index > 0) commands.push("T*"); commands.push(`(${escapePdfText(line)}) Tj`); });
    commands.push("ET"); const content = commands.join("\n");
