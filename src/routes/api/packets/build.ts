...
import { buildPacket, sha256 as computePacketBytesHash, type PacketPageOp, type PacketPart } from "@/platform/packet-builder";
...
export const Route = createFileRoute("/api/packets/build")({ server: { handlers: { POST: async ({ request }) => {
  try {
    ...
    // The Stripe fulfillment webhook reads recipient fields from the stored
    // packet, not from the webhook payload.  Storing them here at build time
    // guarantees the locked packet is self-contained for mailing.
    ...
    const recipientCity = String(form.get("recipientCity") || "").trim();
    const recipientState = String(form.get("recipientState") || "").trim();
    const recipientZip = String(form.get("recipientZip") || "").trim();
    const mailingMethod = String(form.get("mailingMethod") || "standard").trim() as "standard" | "certified" | "registered";
    if (!recipientName || !recipientAddress1 || !recipientCity || !recipientState || !recipientZip) {
      return Response.json({ error: "A complete mailing recipient (name, address1, city, state, zip) is required before packet assembly." }, { status: 400 });
    }
    if (!["standard", "certified", "registered"].includes(mailingMethod)) {
      return Response.json({ error: "mailingMethod must be standard, certified, or registered." }, { status: 400 });
    }
    const recipientHash = await computePacketBytesHash(new TextEncoder().encode(JSON.stringify({ recipientName, recipientAddress1, recipientAddress2, recipientCity, recipientState, recipientZip })));
    ...
    const packet = {
      ...
      status: "assembled",
      locked: true,
      // Recipient — explicitly persisted, not inherited via spread
      recipientName,
      recipientAddress1,
