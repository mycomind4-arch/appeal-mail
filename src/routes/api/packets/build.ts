...
import { buildPacket, type PacketPart } from "@/platform/packet-builder";

interface PacketPartInput {
  id: string;
  type: "ai_response" | "uploaded_document" | "generated_document";
  filename?: string;
  mimeType?: string;
  title?: string;
}
    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          let partInputs: PacketPartInput[];
          ...
          const { data: appeal, error: appealError } = await supabase
            .from("appeals")
            .select("id,user_id,workflow_id,packet")
            .eq("id", appealId)
            .eq("user_id", user.id)
            .single();
          ...
          for (const input of partInputs) {
            ...
            if (input.type === "ai_response") {
              parts.push({ id: input.id, type: "ai_response", text: draft, title: input.title || "Appeal Response" });
              continue;
            }
            ...
            const mimeType = input.mimeType || formValue.type;
            const filename = input.filename || formValue.name;
            const bytes = new Uint8Array(await formValue.arrayBuffer());
            if (input.type === "uploaded_document") {
              parts.push({ id: input.id, type: "uploaded_document", filename, mimeType, bytes });
            } else {
          ...
          const { error: updateError } = await supabase
            .from("appeals")
            .update({ draft, packet, status: "ready", updated_at: assembledAt })
            .eq("id", appealId)
            .eq("user_id", user.id);
          ...
          return Response.json({
            ...
            documentSha256: uploaded.sha256,
            finalDraftHash: built.finalDraftHash,
            attachmentHashes: packet.attachmentHashes,
            pageCount: built.pageCount,
            locked: true,
            assembledAt,
          });
