...
export const Route = createFileRoute("/api/workflows/administrative-decision/approve")({ server: { handlers: { POST: async ({ request }) => {
  try {
    ...
    const packet={id:crypto.randomUUID(),recipientName:r.name,recipientAddress1:r.address1,recipientAddress2:r.address2,recipientCity:r.city,recipientState:r.state,recipientZip:r.zip,mailingMethod:input.mailingMethod,workflowId:"administrative-decision-appeal",finalDocumentType:"response-pdf"};
    ...
    return Response.json({ok:true,appealId:input.appealId,status:"ready",review,packet});
