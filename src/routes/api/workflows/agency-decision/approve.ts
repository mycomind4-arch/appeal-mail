...
export const Route = createFileRoute("/api/workflows/agency-decision/approve")({ server: { handlers: { POST: async ({ request }) => {
  try {
    ...
    const packet = { appealId: appeal.id, workflowId: "agency-decision-appeal", finalResponse: appeal.draft, recipient: input.recipient, mailingMethod: input.mailingMethod, generatedAt: new Date().toISOString(), sourceDecisionId: decision.id, authoritySources: authority, deterministic: true };
    ...
    return Response.json({ ok: true, status: "ready", review, packet });
  } catch (error) {
