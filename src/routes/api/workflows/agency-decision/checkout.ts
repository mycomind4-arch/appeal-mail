...
export const Route = createFileRoute("/api/workflows/agency-decision/checkout")({ server: { handlers: { POST: async ({ request }) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({ mode: "payment", payment_method_types: ["card"], line_items: [{ price_data: { currency: "usd", product_data: { name: LABELS[mailingMethod], description: `Agency decision appeal — ${LABELS[mailingMethod]}` }, unit_amount: PRICES[mailingMethod] }, quantity: 1 }], metadata: { appeal_id: appeal.id, workflow_id: "agency-decision-appeal", mailing_method: mailingMethod, owner_user_id: user.id }, success_url: `${appUrl}/workflows/agency-decision?checkout=success&session_id={CHECKOUT_SESSION_ID}`, cancel_url: `${appUrl}/workflows/agency-decision?checkout=cancelled` });
