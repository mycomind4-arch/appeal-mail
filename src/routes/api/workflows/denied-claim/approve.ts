  ...
  server: {
    ...
    try {
      ...
      if (!recipient?.name || !recipient.address1 || !recipient.city || !recipient.state || !recipient.zip) {
        return Response.json({ error: "A complete mailing recipient is required." }, { status: 400 });
      }
      ...
      if (review.score < 80 || review.issuesRequiringAttention > 2 || review.checks.some((check) => check.status === "fail")) {
        return Response.json({ error: "Appeal is not ready for approval.", review }, { status: 409 });
      }

      const packet = assemblePacket({
        appealId,
        finalLetter: appeal.draft,
        evidence,
        recipient: {
          name: recipient.name,
          address1: recipient.address1,
      ...
      const { error: updateError } = await supabase
        .from("appeals")
        .update({ status: "ready", review, packet, version: currentVersion + 1, updated_at: new Date().toISOString() })
        .eq("id", appealId)
        .eq("user_id", user.id)
