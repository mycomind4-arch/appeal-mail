...
export const APIRoute = createAPIFileRoute("/api/workflows/denied-claim/checkout")({
  POST: async ({ request }) => {
    try {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [{ price_data: { currency: "usd", product_data: { name: LABELS[mailingMethod], description: `Denied claim appeal — ${LABELS[mailingMethod]}` }, unit_amount: PRICES[mailingMethod] }, quantity: 1 }],
        metadata: {
          appeal_id: appeal.id,
          workflow_id: "denied-claim",
          mailing_method: mailingMethod,
          recipient_name: appeal.packet.recipientName,
