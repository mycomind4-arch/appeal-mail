...
export const Route=createFileRoute("/api/workflows/dental-insurance-appeal/checkout")({server:{handlers:{POST:async({request})=>{try{
 ...
 if(a.user_id!==user.id)return Response.json({error:"You do not own this appeal case."},{status:403}); if(a.workflow_id!=="dental-insurance-appeal")return Response.json({error:"Appeal workflow mismatch."},{status:409});
 ...
 const session=await stripe.checkout.sessions.create({mode:"payment",payment_method_types:["card"],line_items:[{price_data:{currency:"usd",product_data:{name:LABELS[method],description:`Dental insurance appeal — ${LABELS[method]}`},unit_amount:PRICES[method]},quantity:1}],metadata:{appeal_id:a.id,workflow_id:"dental-insurance-appeal",mailing_method:method,recipient_name:a.packet.recipientName,owner_user_id:user.id},success_url:`${appUrl}/workflows/dental-insurance-appeal?checkout=success&session_id={CHECKOUT_SESSION_ID}`,cancel_url:`${appUrl}/workflows/dental-insurance-appeal?checkout=cancelled`});
