...
export const Route = createFileRoute("/api/workflows/financial-aid-reinstatement/analyze")({server:{handlers:{POST:async ({request}) => {
  try {
    ...
    const document = await uploadDocument(file); const gemini = await resolveAI("financial-aid-reinstatement", "analysis"); const bytes = Buffer.from(await file.arrayBuffer()).toString("base64");
    const prompt = [
