...
export const APIRoute = createAPIFileRoute("/api/workflows/insurance-denial-letter/checkout")({
  POST: async ({ request }) => {
    try {
      ...
      const session = await stripe.checkout.sessions.create({ mode: "payment", payment_method_types: ["card"], line_items: [{ price_data: { currency: "usd", product_data: { name: LABELS[mailingMethod], description: `Insurance denial letter response — ${LABELS[mailingMethod]}` }, unit_amount: PRICES[mailingMethod] }, quantity: 1 }], metadata: { appeal_id: appeal.id, workflow_id: "insurance-denial-letter", mailing_method: mailingMethod, recipient_name: appeal.packet.recipientName, owner_user_id: user.id }, success_url: `${appUrl}/workflows/insurance-denial-letter?checkout=success&session_id={CHECKOUT_SESSION_ID}`, cancel_url: `${appUrl}/workflows/insurance-denial-letter?checkout=cancelled` });
