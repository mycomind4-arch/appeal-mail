...
async function resolveGemini() {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, {
    ...
    body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "medical-insurance-denial", task: "analysis" }),
  });
...
export const APIRoute = createAPIFileRoute("/api/workflows/medical-insurance-denial/analyze")({
  POST: async ({ request }) => {
    try {
      ...
      if (!(file instanceof File)) return Response.json({ error: "A medical insurance denial is required." }, { status: 400 });
      ...
      const prompt = [
        ...
        "Analyze the actual medical insurance denial document. Separate insurer assertions from facts directly supported by the document.",
        "Identify the denial rationale, coverage or medical-policy references, service or treatment at issue, evidence considered, missing documentation, appeal instructions, deadlines, and factual uncertainty.",
        "Do not invent diagnoses, treatment history, medical necessity, policy language, statutes, regulations, or outcomes. Do not infer a medical fact that is not supported by the uploaded document.",
        "Return strict JSON only.",
        '{"summary":"","decision":"","decisionType":"medical_insurance_denial","issuer":"","referenceNumber":"","decisionDate":"","deadline":"","reasons":[],"keyFacts":[],"medical_or_service_context":[],"issues":[{"issue":"","whyItMatters":"","evidenceNeeded":[]}],"evidenceMentioned":[],"uncertainties":[],"confidence":"high|medium|low"}',
      ].join("\n\n");
      ...
      const analysis = JSON.parse(text) as {
        ...
        reasons?: string[]; keyFacts?: string[]; medical_or_service_context?: string[];
      ...
      const decision = createDecision("claim_denial", {
        ...
        decisionDate: analysis.decisionDate || undefined, decisionTypeLabel: analysis.decision || "Medical insurance denial",
        deadline: analysis.deadline ? { date: analysis.deadline, type: "appeal", source: "extracted" } : undefined,
        facts: [
          ...(analysis.keyFacts || []),
          ...(analysis.medical_or_service_context || []).map((value) => `Service/medical context: ${value}`),
        ].map((value, index) => ({ id: `${index}-${crypto.randomUUID()}`, label: `Fact ${index + 1}`, value, source: "extracted", confidence: 0.8 })),
        reasons: (analysis.reasons || []).map((text, index) => ({ id: `${index}-${crypto.randomUUID()}`, text, confidence: 0.9 })),
        issues: (analysis.issues || []).map((item, index) => ({ id: `${index}-${crypto.randomUUID()}`, description: item.issue || "Issue identified in medical insurance denial", type: "factual_dispute", severity: "medium", sourceExcerpt: item.whyItMatters })),
      ...
      const grounds = (analysis.issues || []).map((issue, index) => createGround("factual_error", {
        ...
        claim: issue.issue || "Review a stated medical insurance finding",
      ...
      evidence.unshift(createEvidence("document", "Original medical insurance denial", { documentId: document.id, documentFilename: document.filename, uploadedAt: new Date().toISOString() }));
      if (evidence.length && grounds.length) grounds[0].supportingEvidenceIds = evidence.map((item) => item.id);

      const appeal = createAppeal("medical-insurance-denial", decision);
      ...
      const message = error instanceof Error ? error.message : "Unable to analyze medical insurance denial.";
