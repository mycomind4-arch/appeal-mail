...
async function resolveGemini(task: "draft" | "validation") {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "medical-insurance-denial", task }) });
...
export const APIRoute=createAPIFileRoute("/api/workflows/medical-insurance-denial/draft")({POST:async({request})=>{try{
  ...
  if(error||!appeal) return Response.json({error:"Appeal case not found."},{status:404}); if(appeal.user_id!==user.id) return Response.json({error:"You do not own this appeal case."},{status:403}); if(appeal.workflow_id!=="medical-insurance-denial") return Response.json({error:"Appeal workflow mismatch."},{status:409});
  const workflow=getWorkflow("medical-insurance-denial"); const draftConfig=await resolveGemini("draft"); const validationConfig=await resolveGemini("validation"); const analysis=input.analysis||appeal.decision;
  const draft=input.draftOverride?.trim()||await callGemini(draftConfig,[`Create a response for the workflow: ${workflow.title}.`,workflow.workflowPrompt,`Focus on: ${workflow.focusAreas.join(", ")}.`,"Use only supplied facts. Do not invent diagnoses, treatment history, policy language, laws, regulations, deadlines, or outcomes. Clearly distinguish documented facts from uncertainty.","Write a professional medical insurance appeal response that a human can review and edit.",`CASE ANALYSIS:\n${JSON.stringify(analysis)}`].join("\n\n"));
  const validation=await callGemini(validationConfig,[`Audit this medical insurance appeal draft for: ${workflow.title}.`,"Return strict JSON with valid, issues, unsupportedClaims, missingEvidence, suggestions.","Flag unsupported medical claims, fabricated policy/legal language, missing clinical/supporting evidence, contradictions, deadline problems, and factual uncertainty.",`CASE ANALYSIS:\n${JSON.stringify(analysis)}`,`DRAFT:\n${draft}`].join("\n\n"));
