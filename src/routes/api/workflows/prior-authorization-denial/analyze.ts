...
export const Route=createFileRoute("/api/workflows/prior-authorization-denial/analyze")({server:{handlers:{POST:async({request})=>{try{
  ...
  const { text: text } = await callLLMDocument(gemini, mediaType(file), bytes, gemini.promptOverride || prompt); const analysis=JSON.parse(text) as any;
  const decision=createDecision("claim_denial",{id:crypto.randomUUID(),documentId:document.id,documentFilename:document.filename,agency:analysis.issuer||undefined,referenceNumber:analysis.referenceNumber||undefined,decisionDate:analysis.decisionDate||undefined,decisionTypeLabel:analysis.decision||"Prior authorization denial",deadline:analysis.deadline?{date:analysis.deadline,type:"appeal",source:"extracted"}:undefined,facts:[...(analysis.clinicalFactsSupported||[]),...(analysis.alternativesMentioned||[])].map((value:string,i:number)=>({id:`${i}-${crypto.randomUUID()}`,label:`Fact ${i+1}`,value,source:"extracted",confidence:.8})),reasons:(analysis.denialReasons||[]).map((text:string,i:number)=>({id:`${i}-${crypto.randomUUID()}`,text,confidence:.9})),issues:(analysis.issues||[]).map((x:any,i:number)=>({id:`${i}-${crypto.randomUUID()}`,description:x.issue||"Prior-authorization issue",type:"factual_dispute",severity:"medium",sourceExcerpt:x.whyItMatters})),rawText:JSON.stringify(analysis),extractedAt:new Date().toISOString(),extractionConfidence:analysis.confidence==="high"?.9:analysis.confidence==="medium"?.7:.5});
  const grounds=(analysis.issues||[]).map((x:any,i:number)=>createGround("factual_error",{id:`ground-${i}-${crypto.randomUUID()}`,claim:x.issue||"Review the prior-authorization finding",source:x.whyItMatters||"Identified by document analysis",confidence:.65,unresolvedIssue:(x.evidenceNeeded||[]).join(", ")}));
