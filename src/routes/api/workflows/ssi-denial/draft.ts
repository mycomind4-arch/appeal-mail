...
export const Route = createFileRoute("/api/workflows/ssi-denial/draft")({server:{handlers:{ POST: async ({ request }) => {
  try {
    ...
    const workflow = getWorkflow("ssi-denial"); const draftConfig = await resolveAI("ssi-denial", "draft"); const validationConfig = await resolveAI("ssi-denial", "validation"); const analysis = input.analysis || appeal.decision;
