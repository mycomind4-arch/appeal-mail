...
import { PackageCheck, Clock, CheckCircle2, FileText, TrendingUp, Mail, ArrowRight, Search } from "lucide-react";
...
const stats = [
  { label: "Total mailings", value: "5", icon: Mail, color: "text-indigo-700" },
  { label: "In transit", value: "1", icon: PackageCheck, color: "text-amber-500" },
  { label: "Delivered", value: "4", icon: CheckCircle2, color: "text-amber-600" },
  { label: "Avg. delivery", value: "3.1 days", icon: Clock, color: "text-slate-400" },
];
const mailings = [
  { id: "AM-2026-0073", type: "Denied Claim Appeal", recipient: "BlueShield Appeals Dept", date: "Aug 11, 2026", status: "in_transit", mailType: "Certified + RR", tracking: "9405 5118 9956 4521 0088" },
  { id: "AM-2026-0068", type: "Government Decision Appeal", recipient: "SSA Office of Appeals", date: "Aug 3, 2026", status: "delivered", mailType: "Certified + RR", tracking: "9405 5118 9956 4498 2210" },
  { id: "AM-2026-0061", type: "Court Ruling Appeal", recipient: "Traffic Court Clerk", date: "Jul 22, 2026", status: "delivered", mailType: "Certified", tracking: "9405 5118 9956 4412 0099" },
  { id: "AM-2026-0055", type: "Reconsideration Request", recipient: "State Farm Reconsideration", date: "Jul 10, 2026", status: "delivered", mailType: "Certified + RR", tracking: "9405 5118 9956 4308 1156" },
  { id: "AM-2026-0049", type: "Denied Claim Appeal", recipient: "Geico Claims Department", date: "Jun 28, 2026", status: "delivered", mailType: "Certified + RR", tracking: "9405 5118 9956 4208 9934" },
];
      ...
      <section className="py-8"><div className="container">
        ...
        <div className="mt-8 card overflow-hidden">
          ...
          <div className="hidden md:block"><table className="w-full text-sm"><thead className="bg-indigo-50 text-left text-xs font-semibold text-indigo-500"><tr><th className="px-5 py-3">Reference</th><th className="px-5 py-3">Type</th><th className="px-5 py-3">Recipient</th><th className="px-5 py-3">Date</th><th className="px-5 py-3">Mail type</th><th className="px-5 py-3">Status</th></tr></thead>
            <tbody className="divide-y divide-warm-border">{mailings.map((m) => (<tr key={m.id} className="hover:bg-cream transition-colors cursor-pointer"><td className="px-5 py-3.5 font-mono text-xs font-semibold text-indigo-700">{m.id}</td><td className="px-5 py-3.5 text-slate-500">{m.type}</td><td className="px-5 py-3.5 text-slate-500">{m.recipient}</td><td className="px-5 py-3.5 text-slate-400">{m.date}</td><td className="px-5 py-3.5 text-slate-400">{m.mailType}</td><td className="px-5 py-3.5"><span className={statusConfig[m.status].badge}>{statusConfig[m.status].label}</span></td></tr>))}</tbody>
          </table></div>
          <div className="divide-y divide-warm-border md:hidden">{mailings.map((m) => (<div key={m.id} className="p-4"><div className="flex items-center justify-between"><span className="font-mono text-xs font-semibold text-indigo-700">{m.id}</span><span className={statusConfig[m.status].badge}>{statusConfig[m.status].label}</span></div><p className="mt-2 font-semibold text-indigo-700">{m.type}</p><p className="mt-1 text-sm text-slate-400">{m.recipient}</p><div className="mt-2 flex items-center gap-3 text-xs text-slate-300"><span>{m.date}</span><span>·</span><span>{m.mailType}</span></div></div>))}</div>
        </div>
        <div className="mt-6 card p-5"><div className="flex items-center gap-2"><PackageCheck size={18} className="text-amber-500" /><h2 className="font-semibold text-indigo-700">Latest tracking</h2></div><p className="mt-1 text-sm text-slate-400">AM-2026-0073 · Certified + Return Receipt</p>
          <div className="mt-4 space-y-4">{[{ date: "Aug 12, 9:30 AM", event: "Mailed from Austin, TX", done: true }, { date: "Aug 12, 2:15 PM", event: "Processed at USPS facility", done: true }, { date: "Aug 13", event: "In transit", done: false }, { date: "—", event: "Out for delivery", done: false }, { date: "—", event: "Delivered (signature required)", done: false }].map((step, i) => (<div key={i} className="flex items-start gap-3"><div className={`mt-0.5 flex h-6 w-6 items-center justify-center rounded-full ${step.done ? "bg-amber-50" : "bg-gray-100"}`}>{step.done ? <CheckCircle2 size={14} className="text-amber-600" /> : <span className="text-[10px] font-bold text-gray-400">{i + 1}</span>}</div><div><p className={`text-sm ${step.done ? "text-indigo-700" : "text-slate-300"}`}>{step.event}</p><p className="text-xs text-slate-300">{step.date}</p></div></div>))}</div>
        </div>
        <div className="mt-6 flex items-center gap-3 rounded-xl border border-dashed border-warm-border bg-white p-5 text-sm text-slate-400"><TrendingUp size={18} className="text-amber-500" /><span>Account features (save drafts, re-send, saved addresses) are coming when authentication launches.</span></div>
      </div></section>
      <SiteFooter />
    </main>
  );
