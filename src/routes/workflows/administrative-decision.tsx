...
function AdministrativeDecisionPage() {
  ...
  return <main className="mx-auto max-w-6xl px-6 py-12">
    <section className="rounded-3xl border border-slate-200 bg-slate-950 p-8 text-white shadow-xl">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">{[["Authority raised","Procedural claims are separated from document facts and tied to official agency, court, statute, regulation, or rule sources."],["Deadline discipline","A date appearing in a decision is not promoted to a filing deadline unless supported by the governing authority."],["Proof-backed fulfillment","The final approved PDF flows through MailMyPDF and retains provider status and tracking/proof."]].map(([title,copy]) => <div key={title} className="rounded-2xl border border-white/10 bg-white/5 p-5"><h2 className="font-semibold">{title}</h2><p className="mt-2 text-sm leading-6 text-slate-300">{copy}</p></div>)}</div>
    </section>
    ...
    <section className="mt-10 rounded-3xl border border-slate-200 bg-white p-7"><h2 className="text-2xl font-semibold">What we refuse to guess</h2><div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{["Deadline","Appeal level","Recipient","Filing portal","Hearing right","Stay rule","Exhaustion","Judicial-review path"].map(item=><div key={item} className="rounded-2xl bg-slate-50 p-4 text-sm font-medium">{item}</div>)}</div><p className="mt-6 text-sm leading-7 text-slate-600">Administrative procedures vary by agency and jurisdiction. Official federal guidance itself shows that distinct administrative regimes can prescribe materially different appeal periods and filing methods.</p></section>
