...
test("license-revocation-appeal is Gold-standard configured", async () => {
  ...
  assert.match(analyze, /uploadDocument/); assert.match(analyze, /control-plane\/ai/); assert.match(analyze, /generateContent/); assert.match(analyze, /supabase\.from\("appeals"\)/); assert.match(analyze, /user_id: user\.id/);
